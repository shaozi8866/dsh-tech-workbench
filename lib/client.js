...
`;
		...
		*/
		function setWorkbenchRenderFunction(fn) {
			renderFunction = fn;
		}
		/**
  ...
  `;
		...
		*/
		function findSidebarEntryContainer(sidebar) {
		...
		* 查找新建会话按钮
		*/
		function findNewSessionButton(container) {
  ...
  `;
		...
		*/
		function ensureWorkbenchView() {
			...
			if (!viewEl) {
				...
				else console.warn("[dsh-tech-workbench] 渲染函数未设置");
			}
			if (!document.body.contains(viewEl)) mainContent.appendChild(viewEl);
		}
		/**
		* 设置工作台显隐状态
		*/
		function setWorkbenchOpen(open) {
			...
			if (entryEl) {
				if (open) entryEl.dataset.active = "true";
				else delete entryEl.dataset.active;
			}
		...
		*/
		function injectEntry() {
			...
			if (!entryEl) entryEl = createSidebarEntry();
			const thirdPartyEntries = Array.from(container.children).filter((e) => e instanceof HTMLElement && e.matches("[data-dsh-taskboard-entry], [data-dsh-ssh-entry], [data-dsh-personal-workbench-entry]"));
			let insertBefore = null;
			if (thirdPartyEntries.length > 0) insertBefore = thirdPartyEntries[thirdPartyEntries.length - 1].nextSibling;
			else if (newSessionBtn) {
				const logoRow = newSessionBtn.closest("[class*=\"logoRow\"]");
				insertBefore = (logoRow && logoRow.parentElement === container ? logoRow : newSessionBtn).nextSibling;
			}
			if (insertBefore) container.insertBefore(entryEl, insertBefore);
			else container.appendChild(entryEl);
			if (!activeObserver) {
				activeObserver = new MutationObserver(() => {
					if (entryEl) {
						if (document.documentElement.hasAttribute(ACTIVE_ATTR)) entryEl.dataset.active = "true";
						else delete entryEl.dataset.active;
					}
				});
		...
		* 启动侧边栏集成
		*/
		function injectSidebarWorkbench() {
			injectGlobalStyles();
			injectEntry();
			ensureWorkbenchView();
			if (!sidebarObserver) {
			...
			if (!cleanupRegistered) {
				...
				document.addEventListener("click", (e) => {
					if (!isOpen) return;
					if (e.target.closest("[class*=\"sessionRow\"], [class*=\"projectRow\"], [class*=\"searchResultRow\"], [class*=\"searchResultWorkspace\"], [class*=\"newSession\"]")) setWorkbenchOpen(false);
				}, true);
			}
		...
		* 完整清理
		*/
		function cleanupSidebarIntegration() {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", {
									...
									}), resource.isOfficial ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: "dshwb-badge-apple dshwb-badge-official-apple",
										children: "官方"
									}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: "dshwb-badge-apple dshwb-badge-community-apple",
										children: "社区"
									})]
								})]
							}),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: !resource.isInstalled ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		...
		*/
		...
		function WorkbenchPage({ type, resources, loading, fromCache, onRefresh, isRefreshing }) {
			...
			const handleUninstall = (resource) => {
				...
				if (confirm(`确定要卸载 ${resource.name} 吗？`)) alert("卸载操作已记录，请重启 DSH 生效。");
			};
			const handleToggleEnable = (resource) => {
  ...
  `;
		...
		function SidebarWorkbenchEntry() {
			const [activeSlot, setActiveSlot] = (0, react.useState)("plugin");
			const slots = [
				{
					id: "plugin",
					label: "插件管理",
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshwb-sidebar-workbench-module",
				children: slots.map((slot) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: `dshwb-sidebar-workbench-item ${activeSlot === slot.id ? "active" : ""}`,
					onClick: () => handleSlotClick(slot.id),
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: { fontSize: "16px" },
						children: slot.icon
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: slot.label })]
				}, slot.id))
			});
		}
		let workbenchReactRoot = null;
		/**
		* 渲染工作台内容到指定容器（供 sidebar-integration.ts 调用）
		*/
		function renderWorkbenchContent(container) {
			try {
				if (workbenchReactRoot) workbenchReactRoot.unmount();
				workbenchReactRoot = (0, react_dom_client.createRoot)(container);
				workbenchReactRoot.render(react.default.createElement(WorkbenchRoot, { initialTab: "plugin" }));
		...
		function apply(ctx) {
			...
			try {
				fetch("/workbench/api/config").then((r) => r.json()).then((json) => {
					if (json.ok && json.version && json.version !== "2.0.0") logger?.warn?.(`[${PLUGIN_ID}] 版本不匹配：Client v${VERSION}，Host v${json.version}，请 Ctrl+Shift+R 强制刷新`);
				}).catch(() => {});
			} catch (err) {}
			try {
		...
		exports.SidebarWorkbenchEntry = SidebarWorkbenchEntry;
		exports.VERSION = VERSION;
		exports.WorkbenchRoot = WorkbenchRoot;
		exports.apply = apply;
		exports.inject = inject;
		exports.name = name;
		exports.renderWorkbenchContent = renderWorkbenchContent;
