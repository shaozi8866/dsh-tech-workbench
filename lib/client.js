...
`;
		...
		*/
		...
		async function apiUpdate(id) {
			try {
				const data = await (await fetch(`/workbench/api/plugin/update/${id}`, { method: "POST" })).json();
				if (data.ok) return {
					ok: true,
					message: `插件 ${id} 已更新到 v${data.data?.to || "最新"}`
				};
				return {
					ok: false,
		...
		async function apiRollback(id, version) {
			try {
				...
				if (data.ok) return {
					ok: true,
					message: `已回退到 v${data.data?.to || version}`
				};
				return {
					ok: false,
		...
		function WorkbenchPage({ type, resources, loading, fromCache, onRefresh, isRefreshing }) {
			...
			const handleConfirmAction = async () => {
				...
				switch (action) {
					case "update":
						result = await apiUpdate(pluginId);
						break;
					case "rollback":
