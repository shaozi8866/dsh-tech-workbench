...
function normalizeSource(source) {
	...
	if (/\s/.test(trimmed)) {
		...
		if (!candidate) throw new Error(`无法从输入中识别包来源（应形如 "dsh plugin add <来源>" 或直接给来源）: ${trimmed}`);
...
*/
function getDisabledPluginIds(opts = {}) {
	try {
...
*/
async function syncLocalEnvStatus(ctx, list, config) {
	...
	return list.map((item) => {
		...
		if (item.type === "plugin") {
			...
			if (!!local || item.isInstalled === true) {
				updated.isInstalled = true;
				updated.isEnabled = !(disabledIds.has(item.id) || disabledIds.has(item.name || "")) && (local ? local.enabled !== false : true);
				if (local) {
...
//#endregion
//#region src/host/changelog-getter.ts
/**
...
function guessRepo(homepage) {
	const text = (homepage || "").trim();
	const m = text.match(/github\.com\/([\w.-]+\/[\w.-]+?)(?:\.git)?\/?$/) || text.match(/^([\w.-]+)\/([\w.-]+)$/);
	if (!m) return null;
	const repo = m[1] === "github.com" ? text.split("github.com/")[1] : `${m[1]}/${m[2]}`;
	return repo ? repo.replace(/\.git$/, "").replace(/\/$/, "") : null;
}
async function readLocal(id, opts) {
	const base = path.join(profileDir(opts), "node_modules", id);
	for (const name of FILE_CANDIDATES) try {
		const file = path.join(base, name);
		if (!(await promises.stat(file)).isFile()) continue;
...
async function fetchRemote(repo) {
	for (const branch of ["main", "master"]) for (const name of ["CHANGELOG.md", "changelog.md"]) {
...
*/
async function getChangelog(id, opts = {}, homepage) {
	if (!id || id.includes("..") || id.startsWith("/") || id.startsWith("\\")) return {
		id,
		found: false,
		source: null
	};
...
async function handleAdminRoute(pathname, method, req, res, deps) {
	...
	const send = (result, statusHint = 200) => {
		...
		if (result.ok && method !== "GET") memoryData = null;
	};
	try {
		if (kind === "plugin") switch (action) {
			...
			case "changelog": {
				const hz = new URL(req.url || "", "http://local").searchParams.get("homepage") || void 0;
				sendJson(res, 200, {
					ok: true,
					data: await getChangelog(target, deps.cliOpts, hz),
					errorCode: null,
					errorMessage: null,
					durationMs: Date.now() - started
				});
