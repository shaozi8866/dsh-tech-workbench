...
### 🛠️ Meta管理·本机维护（v2.4.0 二期新特性）
- **插件生命周期**：安装/卸载/启用/停用/更新/回退（`dsh plugin` 命令转发 + patch 托管区块，启停热生效）
- **Skill 维护**：`~/.dsh/skills` 本机技能的查看/编辑/新建/删除（保存即被 skill-filesystem 监听热更新）
- **预设维护**：agentPresets 名册的查看/编辑组合/复制为新预设/挂载校验/删除（system 预设强制只读）
- **安全护栏**：基座与工作台自身等受保护插件拒绝停用卸载；路径净化防逃逸；写操作限定用户根
- **状态即时更新**：生命周期操作成功后自动强制刷新数据；启停状态以 `cordis.patch.yml` 为唯一真相源（与热生效行为一致）
- **卡片即档案**：每张插件卡就地展开「更新日志」（优先读本地包 CHANGELOG，未安装自动拉 GitHub 仓库）；「官方/社区」徽章即源站链接
