...
const CATEGORY_RULES: { category: string; icon: string; keywords: string[] }[] = [
  // 费用统计（优先级最高）
  { category: '费用统计', icon: '💰', keywords: ['cost', 'usage', 'quota', 'balance', 'wallet', 'meter', 'token usage', '费用', '用量', '配额', '余额', '统计'] },
  // 视觉图像
  ...
  // 会话管理
  { category: '会话管理', icon: '💬', keywords: ['session', 'conversation', 'chat', 'rewind', 'history', 'archive', 'turn', '会话', '对话', '聊天', '回退', '历史', '撤回'] },
  // 通知提醒
  ...
  // 消息编辑
  { category: '消息编辑', icon: '✏️', keywords: ['edit', 'rewrite', 'recall message', 'message', '编辑', '重写', '撤回消息'] },
  // 搜索联网
...
function extractChineseDescription(description: string): string | null {
  // 查找中文部分（中文字符开头，到英文分隔符或结尾）
  const chineseMatch = description.match(/[\u4e00-\u9fa5][\u4e00-\u9fa5a-zA-Z0-9\s，。、；：！？""''（）【】《》\-\.\/+#@%&*=<>~`|\\^[\]{}]*?(?=\s*[·]|\s*[A-Z][a-z]|\s*$)/);
  if (chineseMatch && chineseMatch[0].length > 10) {
    return chineseMatch[0].trim().substring(0, 120);
  }
  return null;
}
...
function generateChineseDescription(id: string, enDescription: string, category: string): string {
  ...
  // 1. 优先提取中文描述
  const chineseDesc = extractChineseDescription(desc);
  if (chineseDesc) {
    return chineseDesc;
  }
...
// 保存结果
const outputPath = path.join(__dirname, 'generated-descriptions-v2.json');
